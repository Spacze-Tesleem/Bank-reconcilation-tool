...
import { Upload, FileSpreadsheet, CheckCircle2, AlertTriangle, FileDown, FileSpreadsheetIcon, FileText, Settings2, RefreshCw, TableIcon, Info, Filter, Search, LayoutList, Minus, Plus, Bell } from 'lucide-react'
...
const DEFAULT_PARSE_OPTS: ParseOptions = { header: true, delimiter: "auto" }
...
/* CSV parsing with options */

async function readCsvWithOptions(file: File, opts: ParseOptions): Promise<ParseResult> {
  return new Promise((resolve, reject) => {
    Papa.parse(file, {
      ...
      complete: (results) => {
        const anyRes = results as any
        const errors = (anyRes.errors || []) as any[]
        if (errors.length > 0) {
        ...
        if (opts.header) {
          const data = (anyRes.data || []) as RawRow[]
          let headers = (anyRes.meta?.fields || Object.keys(data[0] || {})).map((h: any) => (h ?? "").toString().trim())
          // Deduplicate headers
          ...
          // Normalize row keys to the deduped headers
          const rows = data.map((r: RawRow) => {
            const nr: RawRow = {}
            Object.keys(r || {}).forEach((k) => {
              const trimmed = (k ?? "").toString().trim()
              const target = headers.find((h) => h === trimmed || h.startsWith(trimmed))
              nr[target || trimmed] = r[k]
            })
            return nr
          })
          resolve({ headers, rows })
        } else {
          // No header: arrays => objects
          const data = (anyRes.data || []) as any[][]
          const colCount = data.reduce((max: number, row: any[]) => Math.max(max, row?.length || 0), 0)
          const headers = Array.from({ length: colCount }, (_, i) => `Column ${i + 1}`)
          const rows = data.map((arr: any[]) => {
            const obj: RawRow = {}
            headers.forEach((h, i) => {
              obj[h] = arr?.[i] ?? ""
            })
...
/* Excel parsing with options + sheet selection */

async function readExcelWithOptions(
  file: File,
  opts: ParseOptions,
  desiredSheet?: string,
): Promise<{ result: ParseResult; sheetNames: string[]; usedSheet: string }> {
  const XLSX = await import("xlsx")
  const buf = await file.arrayBuffer()
  const wb = XLSX.read(buf, { type: "array" })
  const sheetNames = wb.SheetNames || []
  if (sheetNames.length === 0) {
    return { result: { headers: [], rows: [] }, sheetNames: [], usedSheet: "" }
  }
  const usedSheet = desiredSheet && sheetNames.includes(desiredSheet) ? desiredSheet : sheetNames[0]
  ...
  if (opts.header) {
    const first = (aoa[0] || []).map(trim)
    // Deduplicate headers
    const seen = new Map<string, number>()
    headers = first.map((h: string) => {
      const key = h || "(blank)"
    ...
    const colCount = aoa.reduce((max, row) => Math.max(max, row?.length || 0), 0)
...
function detectMapping(headers: string[]): Mapping {
  ...
  const date = pick(["date", "posted", "txn date", "transaction date", "value date"]) || headers[0] || ""
  const amount = pick(["amount", "amt", "value", "net", "total"])
  const debit = pick(["debit", "withdrawal", "dr", "paid out", "payment", "expense"])
  const credit = pick(["credit", "deposit", "cr", "paid in", "receipt", "income"])
  const description = pick(["description", "details", "narration", "memo", "payee", "particulars", "reference", "ref", "note"])

  let mode: Mapping["mode"] = "amount"
  if (amount) mode = "amount"
  else if (debit && credit) mode = "debitcredit"
  else if (debit || credit) mode = "debitcredit"
...
function summarize(bank: MappedRow[], cashbook: MappedRow[], pairs: MatchPair[]) {
  ...
  const unmatchedBankAmount = round2(
    pairs.filter((p) => !p.matched && p.bank && !p.cashbook).reduce((sum, p) => sum + (p.bank?.amount || 0), 0),
  )
  const unmatchedCashAmount = round2(
    pairs.filter((p) => !p.matched && p.cashbook && !p.bank).reduce((sum, p) => sum + (p.cashbook?.amount || 0), 0),
  )
  return {
    totals: {
...
function reconcileSmart(bank: MappedRow[], cash: MappedRow[], settings: ReconcileSettings) {
  const bankSorted = [...bank].sort((a, b) =>
    a.dateISO === b.dateISO ? a.amount - b.amount : a.dateISO.localeCompare(b.dateISO),
  )
...
export default function Component() {
  ...
  // File modes and sheet states (for Excel)
  const bankMode: FileMode = isExcel(bankFile) ? "excel" : "csv"
  const cashMode: FileMode = isExcel(cashFile) ? "excel" : "csv"
  const [bankSheets, setBankSheets] = useState<string[]>([])
  const [cashSheets, setCashSheets] = useState<string[]>([])
  const [bankSheet, setBankSheet] = useState<string>("")
  const [cashSheet, setCashSheet] = useState<string>("")

  // Parse options
  const [bankOpts, setBankOpts] = useState<ParseOptions>(DEFAULT_PARSE_OPTS)
  const [cashOpts, setCashOpts] = useState<ParseOptions>(DEFAULT_PARSE_OPTS)
  ...
  const resetAll = () => {
    ...
    setAmountMin("")
    setAmountMax("")
    setBankOpts(DEFAULT_PARSE_OPTS)
    setCashOpts(DEFAULT_PARSE_OPTS)
    setBankSheets([])
    setCashSheets([])
    setBankSheet("")
    setCashSheet("")
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: Source) => {
  ...
  // Parse effects: re-parse whenever file, options, or selected sheet change
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      if (!bankFile) {
      ...
      try {
        if (bankMode === "excel") {
          const { result, sheetNames, usedSheet } = await readExcelWithOptions(bankFile, bankOpts, bankSheet)
          if (cancelled) return
          setBankSheets(sheetNames)
          setBankSheet(usedSheet)
          setBankParsed(result)
          setBankMap(detectMapping(result.headers))
          toast({ title: "Bank Excel loaded", description: `${bankFile.name} (${usedSheet}) parsed.` })
        } else {
          const parsed = await readCsvWithOptions(bankFile, bankOpts)
          if (cancelled) return
          setBankParsed(parsed)
          setBankMap(detectMapping(parsed.headers))
          setBankSheets([])
          setBankSheet("")
          toast({ title: "Bank CSV loaded", description: `${bankFile.name} parsed.` })
        }
      } catch (err: any) {
        if (cancelled) return
        setBankParsed(null)
        setBankMap(null)
        setError(err?.message || "Failed to parse bank file")
        toast({ title: "Bank parse error", description: err?.message || "Failed to parse file", variant: "destructive" })
      } finally {
  ...
  }, [bankFile, bankOpts, bankMode, bankSheet])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        if (cashMode === "excel") {
          const { result, sheetNames, usedSheet } = await readExcelWithOptions(cashFile, cashOpts, cashSheet)
          if (cancelled) return
          setCashSheets(sheetNames)
          setCashSheet(usedSheet)
          setCashParsed(result)
          setCashMap(detectMapping(result.headers))
          toast({ title: "Cashbook Excel loaded", description: `${cashFile.name} (${usedSheet}) parsed.` })
        } else {
          const parsed = await readCsvWithOptions(cashFile, cashOpts)
          if (cancelled) return
          setCashParsed(parsed)
          setCashMap(detectMapping(parsed.headers))
          setCashSheets([])
          setCashSheet("")
          toast({ title: "Cashbook CSV loaded", description: `${cashFile.name} parsed.` })
        }
      } catch (err: any) {
        if (cancelled) return
        setCashParsed(null)
        setCashMap(null)
        setError(err?.message || "Failed to parse cashbook file")
        toast({ title: "Cashbook parse error", description: err?.message || "Failed to parse file", variant: "destructive" })
      } finally {
        if (!cancelled) setLoading(false)
      }
    })()
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cashFile, cashOpts, cashMode, cashSheet])
  ...
  const runReconcile = () => {
    if (!canReconcile) {
      toast({
        ...
        description:
          "Resolve validation errors before reconciling. Tip: if Excel, pick the correct Sheet; if CSV, adjust Header/Delimiter; then set Date/Amount mapping.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (res.stats.totals.unmatchedBank + res.stats.totals.unmatchedCash > 0) {
        toast({
          ...
          description:
            "Some entries are unmatched. Increase date window or amount tolerance, or verify sign conventions.",
        })
      } else {
    ...
    <main className="mx-auto w-full max-w-7xl p-4 md:p-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <CardDescription className="text-white/80">
                {"Upload CSV or Excel (.xlsx), validate, map, and reconcile with smart matching."}
              </CardDescription>
            </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <SurfaceCard
          ...
          description="Upload CSV or Excel. For Excel, choose the sheet; for CSV, adjust header/delimiter if needed."
          icon={<FileSpreadsheet className={cn("w-5 h-5", theme.classes.accentText)} />}
          borderClass={theme.classes.accentBorder}
        >
          <Uploader
            id="bank-file"
            file={bankFile}
            ...
            onFileChange={(e) => onFileChange(e, "bank")}
            opts={bankOpts}
            setOpts={setBankOpts}
            mode={bankMode}
            sheetNames={bankSheets}
            sheetName={bankSheet}
            onSelectSheet={setBankSheet}
          />
          {bankParsed && bankMap && <MappingEditor title="Map columns" parsed={bankParsed} mapping={bankMap} onChange={setBankMap} />}
          <ValidationPanel report={bankValidation} label="Bank file" />
        </SurfaceCard>
        ...
        >
          <Uploader
            ...
            onFileChange={(e) => onFileChange(e, "cashbook")}
            opts={cashOpts}
            setOpts={setCashOpts}
            mode={cashMode}
            sheetNames={cashSheets}
            sheetName={cashSheet}
            onSelectSheet={setCashSheet}
          />
          {cashParsed && cashMap && <MappingEditor title="Map columns" parsed={cashParsed} mapping={cashMap} onChange={setCashMap} />}
          <ValidationPanel report={cashValidation} label="Cashbook file" />
        </SurfaceCard>
      </div>
        ...
        <CardContent className="grid gap-4 md:grid-cols-7">
          <div className="grid gap-1.5">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                onChange={(e) => setSettings((s) => ({ ...s, amountTolerance: Number(e.target.value || 0) }))}
              />
          ...
          <div className="grid gap-1.5">
            ...
            <Input
              id="window"
              type="number"
              value={settings.dateWindowDays}
              onChange={(e) => setSettings((s) => ({ ...s, dateWindowDays: Math.max(0, Number(e.target.value || 0)) }))}
            />
          </div>
          <div className="grid gap-1.5">
            ...
            <Select
              ...
              onValueChange={(v: ReconcileSettings["dateFormat"]) => setSettings((s) => ({ ...s, dateFormat: v }))}
            >
              <SelectTrigger>
          ...
          <div className="grid gap-1.5">
            ...
            <Input
              ...
              onChange={(e) => setSettings((s) => ({ ...s, currency: e.target.value.toUpperCase().slice(0, 3) }))}
            />
          </div>
          <div className="grid gap-1.5">
          ...
          <div className="md:col-span-2 flex items-center gap-3">
            <Button
              ...
              onClick={runReconcile}
              aria-disabled={!canReconcile || loading}
              title={!canReconcile ? "Resolve validation and map both files" : "Run reconciliation"}
            >
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-7">
            ...
            <CardContent className="space-y-4">
              ...
              {(stats.totals.unmatchedBank + stats.totals.unmatchedCash > 0 || stats.totals.netDiff !== 0) && (
                <Alert>
                  ...
                  <AlertDescription>
                    Try increasing the date window or amount tolerance. If using Debit/Credit, verify sign convention. Consider searching
                    by description for manual review.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
          ...
          <Card className={cn("md:col-span-4 shadow-md rounded-2xl", theme.classes.accentBorder, "border")}>
            ...
            <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <Metric
                label="Net difference (Bank - Cashbook)"
                value={formatter.format(stats.totals.netDiff)}
                tone={stats.totals.netDiff === 0 ? "emerald" : "amber"}
              />
              ...
              <div className="flex items-center flex-wrap gap-2">
                <Button
                  variant="outline"
                  className="gap-2"
                  onClick={() => downloadCSV(pairs, "reconciliation_all.csv")}
                ...
                <Button
                  variant="outline"
                  className="gap-2"
                  onClick={() => exportExcel(pairs, "reconciliation.xlsx")}
                  disabled={pairs.length === 0}
                >
        ...
        <Card className={cn("mt-4 shadow-lg rounded-2xl", theme.classes.accentBorder, "border")}>
          <CardHeader className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={pagination.prev} disabled={pagination.page === 1}>
                  Prev
                </Button>
                <Button
                  variant="outline"
                  ...
                  return (
                    <RippleContainer key={p.id} className="rounded-lg" colorClass={theme.classes.ripple}>
                      <TableRow className={cn(densityRow, "border-l-4 transition-colors", rowTint, borderTint)}>
                        <TableCell className={cn("font-medium", densityCell)}>
                          {p.bank?.dateISO || <span className="text-muted-foreground">{"—"}</span>}
                        </TableCell>
                        ...
                        <TableCell className={cn("font-medium", densityCell)}>
                          {p.cashbook?.dateISO || <span className="text-muted-foreground">{"—"}</span>}
                        </TableCell>
                        <TableCell className={cn(densityCell)}>
                          <div className="flex items-center gap-2">
          ...
          <CardContent className="py-10 text-center text-muted-foreground">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-sm">
                Upload both files in CSV or Excel (.xlsx). For Excel, pick the sheet. For CSV, ensure header/delimiter are correct. Map
                columns, resolve validation, then click Reconcile.
              </div>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="grid gap-3">
      <div className="grid gap-2">
        <Label htmlFor={id}>File (CSV or .xlsx)</Label>
        <div className="flex items-center gap-2">
          <Input
            id={id}
            ref={inputRef}
        ...
        {file && (
          <div className="text-sm text-muted-foreground">
            Selected: {file.name} {mode === "excel" && sheetName ? `• Sheet: ${sheetName}` : ""}
          </div>
        )}
        {parsed && (
          <div className="text-xs text-muted-foreground">
            Detected {parsed.rows.length} rows • {parsed.headers.length} columns
          </div>
        )}
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {mode === "excel" && sheetNames && sheetNames.length > 0 && onSelectSheet && (
          <div className="grid gap-1.5">
            <Label>Sheet</Label>
            <Select value={sheetName || sheetNames[0]} onValueChange={(v) => onSelectSheet(v)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="grid gap-1.5">
          ...
          <Select value={opts.header ? "yes" : "no"} onValueChange={(v) => setOpts({ ...opts, header: v === "yes" })}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="yes">Yes (first row has column names)</SelectItem>
              <SelectItem value="no">No (generate Column 1, Column 2, ...)</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {mode === "csv" && (
          <div className="grid gap-1.5">
            ...
            <Select value={opts.delimiter} onValueChange={(v: ParseOptions["delimiter"]) => setOpts({ ...opts, delimiter: v })}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      {parsed && (
        <div className="rounded-xl border p-3">
              ...
              <TableHeader>
                <TableRow>
                  {parsed.headers.slice(0, 6).map((h) => (
                    <TableHead key={h}>{h || "(empty header)"}</TableHead>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
          ...
          <div className="text-xs text-muted-foreground mt-2">
            Tip: If columns look shifted or blank, for Excel choose the correct Sheet; for CSV toggle Header or change Delimiter, then
            re-check.
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="grid gap-1.5">
          ...
          <Select value={mapping.date} onValueChange={(v) => onChange({ ...mapping, date: v })}>
            <SelectTrigger aria-label="Date column">
        ...
        <div className="md:col-span-2">
          ...
          <div className="rounded-lg border p-3 mt-1.5">
            <RadioGroup
              value={mapping.mode}
              onValueChange={(v: "amount" | "debitcredit") => onChange({ ...mapping, mode: v })}
            ...
            {mapping.mode === "amount" ? (
              <div className="mt-3 grid gap-1.5">
                <Select value={mapping.amount || ""} onValueChange={(v) => onChange({ ...mapping, amount: v })}>
                  <SelectTrigger aria-label="Amount column">
              ...
              <div className="mt-3 grid grid-cols-3 gap-3">
                <div className="grid gap-1.5">
                  ...
                  <Select value={mapping.debit || ""} onValueChange={(v) => onChange({ ...mapping, debit: v })}>
                    <SelectTrigger aria-label="Debit column">
                      <SelectValue placeholder="Debit col" />
                    </SelectTrigger>
                    <SelectContent>
                      {parsed.headers.map((h) => (
                        <SelectItem key={h} value={h}>
                ...
                <div className="grid gap-1.5">
                  ...
                  <Select value={mapping.credit || ""} onValueChange={(v) => onChange({ ...mapping, credit: v })}>
                    <SelectTrigger aria-label="Credit column">
                      <SelectValue placeholder="Credit col" />
                    </SelectTrigger>
                    <SelectContent>
                      {parsed.headers.map((h) => (
                        <SelectItem key={h} value={h}>
                ...
                <div className="grid gap-1.5">
                  <Label>Sign</Label>
                  <Select
                    value={mapping.signConvention || "credit-positive"}
                    onValueChange={(v: "credit-positive" | "debit-positive") => onChange({ ...mapping, signConvention: v })}
                  >
                    <SelectTrigger aria-label="Sign convention">
                      <SelectValue placeholder="Sign convention" />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <div className="grid gap-1.5">
          ...
          <Select value={mapping.description || ""} onValueChange={(v) => onChange({ ...mapping, description: v })}>
            <SelectTrigger aria-label="Description column">
              <SelectValue placeholder="Description column" />
            </SelectTrigger>
            <SelectContent>
              {parsed.headers.map((h) => (
                <SelectItem key={h} value={h}>
        ...
        return (
          <button
            ...
            className={cn(
              ...
              active ? "bg-white shadow-sm text-emerald-700" : "text-neutral-600 hover:text-neutral-900",
            )}
            aria-pressed={active}
          >
            {opt.label}
